...
<body>
  ...
  <pre>
    ...
    2. get 이라는 단어를 거의 사용하지 않는다!!
      - EL은 표현(출력) 언어
          ->  값을 대입하고, 조건/반복하는 구문을 작성하지 않는다. 
              오로지 출력!!!
              (출력 하려면, 값을 얻어와야 되는게 당연 한거라 get 단어를 사용하지 않음) 
  ...
  <pre>
    - \${param.key} : key가 일치하는 파라미터 얻어오기 <%-- \백슬래시는 무조건 지워줘야함 --%>
    - \${paramValues.key} : key가 일치하는 파라미터 모두 얻어오기 <%-- \백슬래시는 무조건 지워줘야함 --%>
  </pre>
...
<ul>
  ...
  <ul>
    <%-- check가 여러개 전달 된 경우 첫 번째 값을 출력함 --%>
    <li>
  ...
  <%-- check 라는 key를 가진 모든 값을 모아 String[]로 반환해줌 --%>
    <li>
      paramValues.check : ${paramValues.check}
    </li>
    
    <li>
      paramValues.check : ${paramValues.check[0]}
    </li>
    <li>
      paramValues.check : ${paramValues.check[1]}
    </li>
    <li>
      paramValues.check : ${paramValues.check[2]}
    </li>
