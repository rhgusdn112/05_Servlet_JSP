...
<body>
  ...
  <pre>
    * src/main/java
      ...
      - 컴파일 되어있는 상태가 아님
        -> 배포 시 컴파일된 파일만 
          webapp 폴더로 옮기고 나머지는 제외되는 폴더


    * src/main/webapp
      - 실제로 배포되는 웹 어플리케이션의
        컴파일 된 코드, 정적 자원(html,css,js,image)이 
        모여있는 폴더

      - 그래서 파일 경로나 요청 주소를 작성 할 때
        webapp 폴더가 기준이 된다!

      ex) 주소에 / : webapp의 index.jsp 요청
      forward 시 JSP 파일 경로 / : webapp 폴더가 기준
      css, js 파일 경로(==요청 주소) / : webapp 폴더가 기준
    ...
    * src/main/webapp/WEB-INF 폴더
      ...
      - 이럴거면 배포하는 webapp 폴더에 안넣으면 되는거 아닌가?
        -> 직접 해당 파일을 보여줄 순 없지만, 
          응답 화면 등의 웹서비스를 제공하기 위해서는 필요하기 때문에
  </pre>
